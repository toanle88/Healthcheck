...
function computeIncidents(sortedChecks: Check[]) {
...
function computeBuckets(validLatencyChecks: Check[]) {
  const buckets = [
...
const IncidentLogModal: React.FC<IncidentLogModalProps> = ({ target, name, onClose }) => {
  ...
  const incidents = computeIncidents(sortedChecks);

  // Compute Latency Histogram
  const buckets = computeBuckets(validLatencyChecks);
          ...
          <div className="bg-bg-base border border-border-primary/60 rounded-2xl p-5 flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold font-mono text-emerald-500">
                {sortedChecks.length > 0 ? `${(sortedChecks.at(-1)?.uptime_sla ?? 100).toFixed(2)}%` : '100.00%'}
              </p>
            </div>
          </div>
          ...
          <div className="bg-bg-base border border-border-primary/60 rounded-2xl p-6 space-y-6">
            ...
            <div className="space-y-4">
              {buckets.map((bucket) => {
                ...
                return (
                  <div key={bucket.label} className="flex items-center gap-4 text-xs">
            ...
            <table className="w-full text-left border-collapse text-xs">
              ...
              <tbody className="divide-y divide-border-primary/50">
                {history?.map((check) => {
                  ...
                  return (
                    <tr key={`${check.checked_at}-${check.status}`} className="hover:bg-bg-base/50 transition-colors">
